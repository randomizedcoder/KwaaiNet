...
# misses the cache, pulling in numpy and ~235 other derivations (~2.3 GiB)
...
{ pkgsNative }:
_final: _prev: {
  # Crane's writeTOML uses remarshal (JSON → TOML conversion).
  # Dependency chain: remarshal → rich-argparse → rich → markdown-it-py
  #                   → pytest-regressions → numpy  (huge build)
